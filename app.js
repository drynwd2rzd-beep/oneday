...
document.querySelector('.aboutVersion')?.replaceChildren('v0.494');
document.querySelectorAll('.settingValue').forEach(value=>{if(value.textContent?.trim()==='v0.489')value.textContent='v0.494'});
...
showVersionInfo=()=>{closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.494\n\n本版本重点：\n• 全面检查交互与数据流程\n• 清理无效统计代码并加强导入数据安全\n• 修复界面大小设置在恢复/导入后的同步';openModal('infoModal')};
openAbout=()=>{infoTitle.textContent='OneDay';infoText.textContent='记录生活，成为更好的自己。\n\nOneDay v0.494';openModal('infoModal')};
document.querySelector('.meFooter')?.replaceChildren('不是为了记录而记录，',document.createElement('br'),'而是为了更好地生活。',document.createElement('br'),document.createElement('br'),'OneDay v0.494');
