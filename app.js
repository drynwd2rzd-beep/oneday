...
function installNavHitFix(){
  ...
  if(!nav)return;
  // v0.476: native document scroll + native button hit testing.
...
function showVersionInfo(){closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.476\n\n本版本重点：\n• 重构 PWA 首次启动的视口与安全区计算\n• 修复首次打开需触碰一次才恢复布局的问题\n• 底部导航统一根据真实可视高度与安全区定位\n• 页面从后台恢复、旋转屏幕和视口变化时自动重新同步布局\n• 保留 v0.468 的个人中心与数据管理界面';openModal('infoModal')}
...
/* v0.476：iOS PWA 首次启动布局稳定化。
   首帧、页面显示后及视觉视口变化时连续同步导航真实高度，
   不依赖“先触碰一次”才能触发浏览器重新计算布局。 */
/* v0.476：首次启动视口尺寸已在 HTML 首帧锁定。
