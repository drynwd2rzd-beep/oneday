...
function load(){try{return normalize(JSON.parse(localStorage.getItem(KEY)||'null'))}catch{return blankState()}}let S=load(),selected=keyOf(D()),viewMonth=new Date(D().getFullYear(),D().getMonth(),1),reviewMonth=new Date(viewMonth),selectedReviewDate=null,reviewTab='timeline',reviewOverviewMonth=new Date(viewMonth),taskFilter='all',taskTab='today',pendingTheme=S.settings.theme,pendingAccent=S.settings.accent;function save(){S=normalize(S);try{localStorage.setItem(KEY,JSON.stringify(S));return true}catch(error){console.warn('OneDay local storage unavailable',error);toast('本地存储空间不足，请减少照片后再试');return false}}
...
document.querySelector('.aboutVersion')?.replaceChildren('v0.492');
document.querySelectorAll('.settingValue').forEach(value=>{if(value.textContent?.trim()==='v0.489')value.textContent='v0.492'});
...
function showVersionInfo(){closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.490\n\n本版本重点：\n• 重构 PWA 首次启动的视口与安全区计算\n• 修复首次打开需触碰一次才恢复布局的问题\n• 底部导航统一根据真实可视高度与安全区定位\n• 页面从后台恢复、旋转屏幕和视口变化时自动重新同步布局\n• 保留 v0.468 的个人中心与数据管理界面';openModal('infoModal')}
...
showVersionInfo=()=>{closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.492\n\n本版本重点：\n• 优化 iOS 手机端布局、弹层与底部导航\n• 修复首页入口、背景滚动和视口变化问题\n• 增加 PWA 安装配置与本地存储异常提示';openModal('infoModal')};
openAbout=()=>{infoTitle.textContent='OneDay';infoText.textContent='记录生活，成为更好的自己。\n\nOneDay v0.492';openModal('infoModal')};
document.querySelector('.meFooter')?.replaceChildren('不是为了记录而记录，',document.createElement('br'),'而是为了更好地生活。',document.createElement('br'),document.createElement('br'),'OneDay v0.492');
document.documentElement.classList.add('oneday-v0488');applyTheme();save();renderToday();renderTasks();renderPlans();renderReview();renderMe();
