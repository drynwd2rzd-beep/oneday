...
function load(){try{return normalize(JSON.parse(localStorage.getItem(KEY)||'null'))}catch{return blankState()}}let S=load(),selected=keyOf(D()),viewMonth=new Date(D().getFullYear(),D().getMonth(),1),reviewMonth=new Date(viewMonth),selectedReviewDate=null,reviewTab='timeline',reviewOverviewMonth=new Date(viewMonth),taskFilter='all',taskTab='today',pendingTheme=S.settings.theme,pendingAccent=S.settings.accent;function save(){sanitizeIds();S=normalize(S);try{localStorage.setItem(KEY,JSON.stringify(S));return true}catch(error){console.warn('OneDay local storage unavailable',error);toast('本地存储空间不足，请减少照片后再试');return false}}
