...
function installNavHitFix(){
  ...
  const activate=(btn)=>{if(!btn)return;go(btn.dataset.go)};
  buttons.forEach(b=>{
    b.onclick=()=>activate(b);
    b.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();activate(b)},{passive:false});
  });
  // iOS PWA fallback：即使页面存在透明/错位命中层，只要手指落在可见导航区域，
  // 捕获阶段仍按横向分区执行导航，避免“导航放大后看得见但点不到”。
  let lastTap=0;
  const routeByPoint=(x,y)=>{
    const r=nav.getBoundingClientRect();
    if(x<r.left||x>r.right||y<r.top||y>r.bottom)return false;
    const i=Math.max(0,Math.min(buttons.length-1,Math.floor((x-r.left)/(r.width/buttons.length))));
    activate(buttons[i]);
    return true;
  };
  document.addEventListener('pointerup',e=>{
    ...
    if(routeByPoint(e.clientX,e.clientY)){lastTap=now;e.preventDefault();}
  },true);
  document.addEventListener('touchend',e=>{
    ...
    const now=Date.now();
    if(now-lastTap<250)return;
    if(routeByPoint(t.clientX,t.clientY)){lastTap=now;e.preventDefault();}
...
function showVersionInfo(){closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.475\n\n本版本重点：\n• 重构 PWA 首次启动的视口与安全区计算\n• 修复首次打开需触碰一次才恢复布局的问题\n• 底部导航统一根据真实可视高度与安全区定位\n• 页面从后台恢复、旋转屏幕和视口变化时自动重新同步布局\n• 保留 v0.468 的个人中心与数据管理界面';openModal('infoModal')}
...
/* v0.475：iOS PWA 首次启动布局稳定化。
   首帧、页面显示后及视觉视口变化时连续同步导航真实高度，
   不依赖“先触碰一次”才能触发浏览器重新计算布局。 */
/* v0.475：首次启动视口尺寸已在 HTML 首帧锁定。
