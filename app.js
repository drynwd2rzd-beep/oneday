...
const UI_SIZE_LABELS={compact:'紧凑',standard:'标准',large:'大'};function currentUiSize(){return UI_SIZE_LABELS[S.settings.uiSize]?S.settings.uiSize:'standard'}function applyUiSize(){const size=currentUiSize();S.settings.uiSize=size;document.documentElement.dataset.uiSize=size;const label=document.getElementById('uiSizeLabel');if(label)label.textContent=UI_SIZE_LABELS[size];const quickLabel=document.getElementById('uiSizeQuickLabel');if(quickLabel)quickLabel.textContent=UI_SIZE_LABELS[size];document.querySelectorAll('[data-ui-size-choice]').forEach(button=>button.classList.toggle('active',button.dataset.uiSizeChoice===size))}function openUiSize(){applyUiSize();openModal('uiSizeModal')}function saveUiSize(){const selectedSize=document.querySelector('[data-ui-size-choice].active')?.dataset.uiSizeChoice;if(!UI_SIZE_LABELS[selectedSize])return;S.settings.uiSize=selectedSize;applyUiSize();save();closeModal('uiSizeModal');toast('界面大小已保存')}function setUiSize(size){if(!UI_SIZE_LABELS[size])return;S.settings.uiSize=size;applyUiSize();save();toast(`界面大小：${UI_SIZE_LABELS[size]}`)}function sanitizeIds(){[...S.tasks,...S.plans].forEach(item=>{const safe=String(item.id||'').replace(/[^A-Za-z0-9_-]/g,'').slice(0,80);item.id=safe||uuid()})}
...
document.querySelector('.aboutVersion')?.replaceChildren('v0.503');
document.querySelectorAll('.settingValue').forEach(value=>{if(value.textContent?.trim()==='v0.489')value.textContent='v0.503'});
...
function showVersionInfo(){closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.503\n\n本版本更新：\n• 优化回顾左滑删除区域的圆角衔接，避免突兀的直角切口\n• 界面大小选中状态改为低调的中性高亮，不再使用绿色\n• IndexedDB 自动保存，保留本地备份机制\n• 保留 JSON 数据导入与导出，方便个人备份和换设备\n• 增加 GitHub Pages HTTPS 下的离线缓存支持\n• 优化 iOS 动态视口、安全区、键盘和主屏幕 Web App 适配\n• 修复固定导航、弹层和回顾左滑删除的交互问题\n\n使用建议：\n• 更换设备或大版本更新前，先在“数据管理”中导出备份\n• iPhone 请使用 Safari 添加到主屏幕';openModal('infoModal')}
...
showVersionInfo=()=>{closeModal('aboutModal');infoTitle.textContent='版本信息';infoText.textContent='OneDay v0.503\n\n本版本更新：\n• 优化回顾左滑删除区域的圆角衔接，避免突兀的直角切口\n• 界面大小选中状态改为低调的中性高亮，不再使用绿色\n• IndexedDB 自动保存，保留本地备份机制\n• 保留 JSON 数据导入与导出，方便个人备份和换设备\n• 增加 GitHub Pages HTTPS 下的离线缓存支持\n• 优化 iOS 动态视口、安全区、键盘和主屏幕 Web App 适配\n• 修复固定导航、弹层和回顾左滑删除的交互问题\n\n使用建议：\n• 更换设备或大版本更新前，先在“数据管理”中导出备份\n• iPhone 请使用 Safari 添加到主屏幕';openModal('infoModal')};
openAbout=()=>{infoTitle.textContent='OneDay';infoText.textContent='记录生活，成为更好的自己。\n\nOneDay v0.503';openModal('infoModal')};
document.querySelector('.meFooter')?.replaceChildren('不是为了记录而记录，',document.createElement('br'),'而是为了更好地生活。',document.createElement('br'),document.createElement('br'),'OneDay v0.503');
openHelp=()=>{infoTitle.textContent='使用说明';infoText.textContent='今天：记录当天的想法并完成事项。\n事项：查看今日、日历和完成统计。\n计划：管理长期方向，可完成或归档。\n回顾：只回看真实记录，不把人生变成任务成绩单。\n\n在 iPhone 上使用：用 Safari 打开 GitHub Pages 地址，点分享 → 添加到主屏幕，并选择“作为 Web App 打开”。首次打开联网一次后，之后可在离线状态下继续使用。';openModal('infoModal')};
